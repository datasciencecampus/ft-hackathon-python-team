...
def start_game():
    ...
    def check_win(root, WORD, target_word, NUM_GUESSES, GUESS_NUM):
        ...
        if WORD == target_word:
            ...
            message = CTkMessagebox(title = "Try again?",message = "Try again?", icon = "question", option_1 = "Yes", option_2 = "Nah")
            response = message.get()
            if response == "Yes":
                quit_game(root)
                GUESS_NUM = 1 # <- problem here
                start_game()
            elif response == "Nah":
                quit_game(root)
    ...
    def key_pressed(event):
        ...
        if key in ALPHABET and LETTER_COUNT < WORD_LENGTH:
            ...
            if not get_definition(WORD):
                ...
                root.after(2500, check_win, root, WORD, target_word,GUESS_NUM,NUM_GUESSES)
